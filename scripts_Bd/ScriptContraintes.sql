...
ADD CONSTRAINT PK_PERSONNAGE PRIMARY KEY (NoPersonnage),  -- Cle primaire
    ...
    CONSTRAINT CHK_PERSONNAGE_Level CHECK (Levels > 0 AND Levels <= 5); -- Contrainte CHECK pour vérifier que level se trouve dans l'intervale voulu
...
ADD CONSTRAINT PK_ARME PRIMARY KEY (NoArme),  -- Cle primaire
    CONSTRAINT CHK_ARME_Niveau CHECK (Niveau >= 1),  -- Contrainte CHECK pour Niveau, Le niveau doit etre au moins 1
    CONSTRAINT CHK_ARME_Degats CHECK (Degats >= 0);  -- Contrainte CHECK pour Degats, Les degats ne peuvent pas etre negatifs
...
ADD CONSTRAINT PK_ARMURE PRIMARY KEY (NoArmure),  -- Cle primaire
    CONSTRAINT CHK_ARMURE_Niveau CHECK (Niveau >= 1),  -- Contrainte CHECK pour Niveau, Le niveau doit etre au moins 1
    CONSTRAINT CHK_ARMURE_Resistance CHECK (Resistance >= 0);  -- Contrainte CHECK pour Resistance, La resistance ne peut pas etre negative
...
ADD CONSTRAINT PK_NIVEAU PRIMARY KEY (Levels),  -- Cle primaire
    CONSTRAINT CHK_NIVEAU_ExpRequis CHECK (ExpRequis >= 0),  -- Contrainte CHECK pour ExpRequis, L'experience requise ne peut pas etre negative
    CONSTRAINT CHK_NIVEAU_GainVie CHECK (GainVie >= 0);  -- Contrainte CHECK pour GainVie, Le gain de vie doit etre positif ou nul
...
ADD CONSTRAINT FK_CLASSE_NoArme FOREIGN KEY (NoArme) REFERENCES ARME(NoArme),  -- Cle etrangere vers ARME
	CONSTRAINT FK_CLASSE_NoArmure FOREIGN KEY (NoArmure) REFERENCES ARMURE(NoArmure);  -- Cle etrangere vers ARMURE
GO
...
ADD CONSTRAINT FK_PROFIL_NoPersonnage FOREIGN KEY (NoPersonnage) REFERENCES PERSONNAGE(NoPersonnage) ON DELETE CASCADE,  -- Cle etrangere avec suppression en cascade
	CONSTRAINT FK_PROFIL_NoClasse FOREIGN KEY (NoClasse) REFERENCES CLASSE(NoClasse) ON DELETE CASCADE;  -- Cle etrangere avec suppression en cascade
GO
